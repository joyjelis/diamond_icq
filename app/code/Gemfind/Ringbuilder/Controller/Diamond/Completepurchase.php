...
class Completepurchase extends Action
{
    const ATTRIBUTE_SET_DIAMONDS = 'Gemfind Diamonds';

    const ATTRIBUTE_SET_RING = 'Gemfind Ringbuilder';

    const COOKIE_NAME_DIAMOND = 'diamondsetting';
     ...
     * @param OptionFactory $optioninterface
    ...
    public function execute()
    {
        ...
        try {
            ...
            if(isset($diamondData['diamondData']['shape'])){
              ...
              $urlstring = strtolower($urlshape.$urlcarat.$urlcolor.$urlclarity.$urlcut.$urlcertificate.'sku-dl-'.$diamondData['diamondData']['diamondId']);
                ...
                if (isset($imageurl)) {
                    ...
                    if (!$this->is_404($imageUrl)) {
                        if ($imageUrl) {
                            ...
                            $filename = strtolower(str_replace(' ', '_', $diamondName)) . $diamondData['diamondData']['diamondId'] . '.' . $imageType;
            ...
            $price = $diamondData['diamondData']['fltPrice'];
    ...
    {
        $attributeSetId = $this->attributeSet->load(self::ATTRIBUTE_SET_RING, 'attribute_set_name')->getAttributeSetId();

        if (!$attributeSetId) {
            throw new \Magento\Framework\Exception\LocalizedException(
                __("Invalid Attribute Set")
            );
        }
            ...
            if (isset($imageurl)) {
                ...
                if (!$this->is_404($imageUrl)) {
                    if ($imageUrl) {
                        $imageType = substr(strrchr($imageUrl, "."), 1);
                        //$filename = md5($imageUrl . $diamondName) . '.' . $imageType;
                        $filename = strtolower(str_replace(' ', '_', $diamondName)) . $ringData['ringData']['settingId'] . '.' . $imageType;
                        $tmpDir = $this->directoryList->getPath(DirectoryList::MEDIA) . DIRECTORY_SEPARATOR . 'tmp';
                        $this->file->checkAndCreateFolder($tmpDir);
                        $localImageUrl = $tmpDir . DIRECTORY_SEPARATOR . $filename;
                        file_put_contents($localImageUrl, file_get_contents(trim($imageUrl)));
