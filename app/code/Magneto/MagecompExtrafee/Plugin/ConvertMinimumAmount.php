...
class ConvertMinimumAmount
{
    ...
    public function aroundGetMinimumOrderAmount(ExtrafeeHelper $subject, $proceed)
    {
        
        $isEnabled = $proceed();
        $isEnabled = $this->convertGetMinimumOrderAmount();
          return $isEnabled;
        
        //return $proceed($configPath, $sendTo, $data);
    }

    public function convertGetMinimumOrderAmount()
    {
        
        $logger = \Magento\Framework\App\ObjectManager::getInstance()->get('\Psr\Log\LoggerInterface');
        $logger->debug("override for converting minimum amount currency"); // add logs in debug.log

        $storeScope = \Magento\Store\Model\ScopeInterface::SCOPE_STORE;
        $currencycode = $this->storeManager->getStore()->getCurrentCurrency()->getCode();
        $newConvertedPrice  = $this->myHelper->convertPrice($this->scopeConfig->getValue(self::CONFIG_MINIMUM_ORDER_AMOUNT, $storeScope), null, $currencycode);
        $logger->debug("Our new converted minimum price : ".$newConvertedPrice); // add logs in debug.log
