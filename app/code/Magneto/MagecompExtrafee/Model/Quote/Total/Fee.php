<?php
namespace Magneto\MagecompExtrafee\Model\Quote\Total;
...
class Fee extends \Magento\Quote\Model\Quote\Address\Total\AbstractTotal
{
    ...
    public function __construct(
        ...
        \Magneto\MagecompExtrafee\Helper\Data $MyHelper
    ) {
    ...
    public function collect(
        ...
        $logger->debug('Model Override Collect Test after using di');


        $enabled = $this->helperData->isModuleEnabled() && $this->myHelper->getIsProductsEngraved($quote);
        $minimumOrderAmount = $this->helperData->getMinimumOrderAmount();
        $logger->debug('getMinimumOrderAmount : '.$minimumOrderAmount);
        $subtotal = $total->getTotalAmount('subtotal');
        $logger->debug('subtotal : '.$subtotal);
        if ($enabled && $subtotal < $minimumOrderAmount) {

            $logger->debug('at fees show in the');

            $fee = $this->helperData->getExtrafee();

            $logger->debug('at extra fees : '.$fee);
            ...
            if ($version > 2.1) {
                $logger->debug(' -- comes at version >2.1');
    ...
    public function fetch(\Magento\Quote\Model\Quote $quote, \Magento\Quote\Model\Quote\Address\Total $total)
    {
        ...
        $logger = \Magento\Framework\App\ObjectManager::getInstance()->get('\Psr\Log\LoggerInterface');
        $logger->debug('Model Override comes to fetch after using di');
        ...
        $result = [];
        if ($enabled && ($subtotal < $minimumOrderAmount ) && $fee) {
            $result = [
                'code' => 'fee',
